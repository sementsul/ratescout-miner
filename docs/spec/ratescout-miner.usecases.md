...
## UC-7. Автосохранение настроек — 🔴
**Ожидаемо:** пул/кошелёк/воркер/CPU% сохраняются в `%LocalAppData%\RateScoutMiner\settings.json` при «Старт» и
при закрытии окна; при следующем запуске поля подставляются. **РАДИУС:** `Settings.cs`, `MainForm` (Load/SaveSettings).

## UC-8. Визуал: подписи над полями — 🔴
**Проблема (со скрина):** длинная подпись «XMR-кошелёк (куда майнить):» налезала на поле ввода.
**Ожидаемо:** раскладка stacked — подпись ОТДЕЛЬНОЙ строкой над полноширинным полем; налезаний нет. **РАДИУС:** `MainForm.StackRow`.

## UC-9. Фикс скачивания XMRig — 🔴
**Проблема:** фильтр искал `msvc-win64`/`gcc-win64`, а у XMRig ассеты `…-windows-x64.zip` / `…-windows-gcc-x64.zip`
→ ничего не качалось («Не нашёл Windows-сборку»). **Ожидаемо:** матчим `windows`+`x64`+`.zip` (без arm), предпочитая MSVC.
**РАДИУС:** `XmrigManager.EnsureXmrigAsync`.
